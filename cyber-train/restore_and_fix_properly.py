#!/usr/bin/env python3
"""
PROPER FIX: Restore from best backup, then carefully fix boundaries and add ONLY legitimate missing entities.
This will NOT add false positives.
"""
...
# Strict patterns - only match when we're confident
...
LATITUDE_PATTERN = re.compile(r'\b-?([0-8]?[0-9](?:\.\d+)?|90(?:\.0+)?)\s*(?:degrees?|°|lat)\b', re.IGNORECASE)
LONGITUDE_PATTERN = re.compile(r'\b-?(?:[0-9]?[0-9]?[0-9](?:\.\d+)?|1[0-7][0-9](?:\.\d+)?|180(?:\.0+)?)\s*(?:degrees?|°|lon|lng)\b', re.IGNORECASE)
...
THREAT_ACTORS = {
    ...
    'Fancy Bear', 'Equation Group', 'Sandworm', 'Turla', 'APT1', 'APT3',
}
...
def fix_entity_boundary(text: str, start: int, end: int, label: str) -> Tuple[int, int]:
    """Fix entity boundary - trim whitespace, fix partial words."""
    # Trim whitespace
...
def extract_legitimate_entities(text: str, existing_spans: Set[Tuple[int, int]]) -> List[List]:
    """Extract ONLY legitimate entities that are clearly missing."""
    found = []
    ...
    # Phone numbers (be careful - only if clearly a phone)
    for match in PHONE_PATTERN.finditer(text):
        span = (match.start(), match.end())
        if span not in existing_spans:
            # Check context - don't match random numbers
    ...
    # GitHub commits (7-40 char hex)
    for match in GITHUB_COMMIT_PATTERN.finditer(text):
        span = (match.start(), match.end())
        if span not in existing_spans:
            # Check context suggests it's a commit
            context_start = max(0, match.start() - 30)
            context_end = min(len(text), match.end() + 30)
            context = text[context_start:context_end].lower()
            if any(word in context for word in ['commit', 'hash', 'sha', 'git']):
                found.append([match.start(), match.end(), 'GITHUB_COMMIT'])
                existing_spans.add(span)
    
    # IP addresses (only if clearly an IP)
    for match in IP_PATTERN.finditer(text):
        ...
        if span not in existing_spans:
            ...
            context_end = min(len(text), match.end() + 30)
            context = text[context_start:context_end].lower()
            if any(word in context for word in ['ip', 'address', 'host', 'server', 'attacker', 'source', 'destination']):
                found.append([match.start(), match.end(), 'IP_ADDRESS'])
                existing_spans.add(span)
    
    # Threat actors (exact matches only)
...
def process_file(file_path: Path) -> Dict:
    """Process file: restore from backup, fix boundaries, add legitimate entities."""
    # Find best backup
    ...
    # Create new backup
    backup_path = file_path.with_suffix(file_path.suffix + f'.backup_restore_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
...
def main():
    ...
    print("="*80)
    print("PROPER FIX: RESTORE FROM BACKUP + FIX BOUNDARIES + ADD LEGITIMATE ENTITIES")
    print("="*80)
