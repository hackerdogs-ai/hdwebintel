...
1. Restore from .backup file (if exists) or best backup
2. Fix boundaries (ONLY trim whitespace)
3. Add ONLY clearly legitimate missing entities
...
def find_best_backup(file_path: Path) -> Path:
    """Find best backup - prefer .backup, then most entities."""
    # First try simple .backup
    simple_backup = file_path.parent / f"{file_path.stem}.backup"
    if simple_backup.exists():
        return simple_backup
    
    # Then try .backup2
    backup2 = file_path.parent / f"{file_path.stem}.backup2"
    if backup2.exists():
        return backup2
    
    # Then find backup with most entities
                ...
                for i, line in enumerate(f):
                    ...
                    if line.strip():
                        try:
                            ...
                            count += len(entities)
                            if entities:
                                sample_labels.extend([e[2] for e in entities[:2]])
                        except:
                            pass
            # Prefer backups with diverse labels (not all GITHUB_USER)
            github_user_ratio = sample_labels.count('GITHUB_USER') / len(sample_labels) if sample_labels else 1.0
            if github_user_ratio < 0.5:  # Less than 50% GITHUB_USER
...
def fix_entity_boundary(text: str, start: int, end: int, label: str) -> Tuple[int, int]:
    """Fix boundary - ONLY trim whitespace."""
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                ...
                # Fix boundaries (ONLY trim whitespace)
                fixed_entities = []
                existing_spans = set()
                ...
                # Add legitimate missing entities
...
def main():
    ...
    print("="*80)
    print("FINAL FIX: RESTORE + FIX BOUNDARIES + ADD LEGITIMATE ENTITIES")
    print("="*80)
